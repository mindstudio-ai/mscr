export const handler = async ({
  ...
  if (!consumerKey) {
    throw new Error("Missing WooCommerce Consumer Key in environment variables");
  }
  if (!consumerSecret) {
    throw new Error("Missing WooCommerce Consumer Secret in environment variables");
  }
  ...
  try {
    ...
    log(`Error creating shipping class: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
};