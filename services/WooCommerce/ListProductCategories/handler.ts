...
export const handler = async ({
  ...
  if (!url) {
    throw new Error('Missing Store URL. Please configure your WooCommerce connection.');
  }
  
  if (!consumerKey || !consumerSecret) {
    throw new Error('Missing API credentials. Please configure your WooCommerce API keys.');
  }
  ...
  if (hideEmpty) queryParams.append('hide_empty', hideEmpty);
  ...
  try {
    ...
    // Handle any errors
    log(`Error fetching product categories: ${error instanceof Error ? error.message : String(error)}`);
    throw error;
  }
};