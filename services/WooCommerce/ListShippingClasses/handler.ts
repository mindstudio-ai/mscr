export const handler = async ({
  ...
  if (!outputVariable) {
    throw new Error(
      'Missing output variable name. Please specify a variable name to store the results.',
    );
  }
  ...
  try {
    ...
    if (!Array.isArray(shippingClasses)) {
      throw new Error(
        'Unexpected response format. Expected an array of shipping classes.',
      );
    }
    ...
    log(
      `Error fetching shipping classes: ${error instanceof Error ? error.message : String(error)}`,
    );
    throw error;
  }
};
