export const handler = async ({
  ...
  // Extract inputs
  const { createClasses, updateClasses, deleteClassIds, outputVariable } = inputs;
  
  // Prepare the request body
  const requestBody: Record<string, any> = {};
  
  // Add create operations if provided
  if (createClasses && Array.isArray(createClasses) && createClasses.length > 0) {
  ...
  if (updateClasses && Array.isArray(updateClasses) && updateClasses.length > 0) {
  ...
  if (deleteClassIds) {
    ...
    const deleteIds = deleteClassIds.split(',')
  ...
  if (Object.keys(requestBody).length === 0) {
    throw new Error("No operations specified. Please provide at least one create, update, or delete operation.");
  }
  ...
  try {
    ...
    log(`Operation completed successfully: ${createdCount} created, ${updatedCount} updated, ${deletedCount} deleted`);
