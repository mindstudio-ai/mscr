...
export const handler = async ({
  ...
  try {
    ...
    log(
      `Successfully deleted category: ${deletedCategory.name} (ID: ${deletedCategory.id})`,
    );
    ...
    if (error.response) {
      ...
      if (statusCode === 404) {
        throw new Error(
          `Category with ID ${categoryId} not found. Please check the category ID and try again.`,
        );
