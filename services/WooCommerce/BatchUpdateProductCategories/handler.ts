export const handler = async ({
  ...
  if (!url) {
    throw new Error(
      'Store URL is required. Please configure your WooCommerce store URL.',
    );
  }

  if (!consumerKey || !consumerSecret) {
    throw new Error(
      'WooCommerce API credentials are required. Please configure your Consumer Key and Consumer Secret.',
    );
  }
  ...
  if (
    createCategories &&
    Array.isArray(createCategories) &&
  ...
  try {
    ...
    log(
      `Successfully processed categories: ${createdCount} created, ${updatedCount} updated, ${deletedCount} deleted`,
    );
