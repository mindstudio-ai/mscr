export const handler = async ({
  ...
  if (parent !== undefined) requestBody.parent = parseInt(parent, 10);
  if (description !== undefined) requestBody.description = description;
  if (display !== undefined) requestBody.display = display;
  if (menuOrder !== undefined) requestBody.menu_order = parseInt(menuOrder, 10);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to update category: ${response.status} ${response.statusText} - ${errorText}`);
    }
    ...
    // Log success
    log(`Successfully updated category: ${updatedCategory.name} (ID: ${updatedCategory.id})`);
