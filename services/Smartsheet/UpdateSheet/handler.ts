...
export const handler = async ({
  ...
  try {
    ...
    if (inputs.projectSettings !== undefined) {
      requestBody.projectSettings = inputs.projectSettings;
    }
    if (inputs.userSettings !== undefined) {
      requestBody.userSettings = inputs.userSettings;
    }

    const response = await smartsheetApiRequest({
      method: 'PUT',
      path: `/sheets/${inputs.sheetId}`,
      queryParams,
      body: requestBody,
    });
