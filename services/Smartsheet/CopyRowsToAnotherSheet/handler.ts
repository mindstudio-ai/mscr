import { CopyRowsToAnotherSheetInputs } from './type';
...
export const handler = async ({
  ...
  setOutput,
  log,
}: IHandlerContext<CopyRowsToAnotherSheetInputs>) => {
  if (!inputs.sheetId) {
  ...
  try {
    ...
    throw new Error(`Failed to copy rows to another sheet: ${errorMessage}`);
  }
};
